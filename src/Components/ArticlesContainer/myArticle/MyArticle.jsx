...
import  style from './MyArticle.module.css';
...
const MyArticle = (props) => {
  ...
  const [recipeDescription, setDescription] = useState(props.description)
  ...
  const deactivateEditMode = () => {
    debugger
    props.updateArticleMenuThunkCreator(props.id, recipeTitle, recipeText, recipeDescription,  props.categoryId)
    setEditMode(false);
  };
const editRecipeTitle = (event) => {
  setRecipeTitle(event.target.value)
};
  const editRecipeDescription = (event) => {
  ...
  const removeRecipe = () => {
    props.deleteArticleMenuThunkCreator(props.id, props.categoryId )
  }
  ...
  return (
    <form onSubmit={props.handleSubmit} className={style.formEditing}>
      <div className={style.recipe}>
        ...
        {editMode? <input type='textarea' className={style.descriptionField} value={recipeDescription} onChange={editRecipeDescription} /> : <div className={style.description}>{props.description}</div>}
        ...
        <img src={article} alt="img" className={style.img}/>
      </div>
    </form>
